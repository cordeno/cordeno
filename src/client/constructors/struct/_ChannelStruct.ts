...
export class ChannelStruct {
  ...
  async bulkDelete(
    ...
    options?: { filterTwoWeeks: Boolean },
  ) {
    messages = options?.filterTwoWeeks
      ? messages.filter(
        ((id) =>
          Date.now() - Snowflake.parse(String(id)).timestamp < 1209600000),
      ).splice(0, 100)
  ...
  async getMessages(
    ...
    return await this.client.http.get(
      `/channels/${this.id}/messages${limit ? `?limit=${limit}` : `?limit=50`}${
        options?.before ? `&before=${options.before}` : ""
      }${options?.after ? `&after=${options?.after}` : ""}${
        options?.around ? `&around=${options}` : ""
      }`,
    );
  }
}
